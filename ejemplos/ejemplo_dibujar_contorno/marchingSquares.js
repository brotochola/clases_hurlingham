...
(function (window) {
  ...
  // the boundary.
  MarchingSquares.getBlobOutlinePoints = function (sourceCanvas, threshold) {
    MarchingSquares.threshold=threshold
    //Add a padding of 1 pixel to handle points which touch edges
    MarchingSquares.sourceCanvas = document.createElement("canvas");
    MarchingSquares.sourceCanvas.willReadFrequently=true
    MarchingSquares.sourceCanvas.width = sourceCanvas.width + 2;
    MarchingSquares.sourceCanvas.height = sourceCanvas.height + 2;
    MarchingSquares.sourceContext =
      MarchingSquares.sourceCanvas.getContext("2d", { willReadFrequently: true });
    MarchingSquares.sourceContext.drawImage(sourceCanvas, 1, 1);

    // Find the starting point
    var startingPoint = MarchingSquares.getFirstNonTransparentPixelTopDown(
      MarchingSquares.sourceCanvas
    );
  ...
  MarchingSquares.getFirstNonTransparentPixelTopDown = function (canvas) {
    ...
    for (y = 0; y < canvas.height; y++) {
      ...
      for (i = 0; i < rowData.length; i += 4) {
        if (rowData[i + 3] > MarchingSquares.threshold) {
          return { x: i / 4, y: y };
        }
      }
    }
  ...
  MarchingSquares.walkPerimeter = function (startX, startY) {
    ...
    var imageData = MarchingSquares.sourceContext.getImageData(
      0,
      0,
      MarchingSquares.sourceCanvas.width,
      MarchingSquares.sourceCanvas.height
    );
    var index,
      width4 = imageData.width * 4;
  ...
  MarchingSquares.step = function (index, data, width4) {
    ...
    MarchingSquares.upLeft = data[index + 3] > MarchingSquares.threshold;
    MarchingSquares.upRight = data[index + 7] > MarchingSquares.threshold;
    MarchingSquares.downLeft =
      data[index + width4 + 3] > MarchingSquares.threshold;
    MarchingSquares.downRight =
      data[index + width4 + 7] > MarchingSquares.threshold;
