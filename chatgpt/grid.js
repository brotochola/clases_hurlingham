class Grid {
  ...
  add(objeto) {
    const xIndex = Math.floor(objeto.sprite.x / this.cellSize);
    const yIndex = Math.floor(objeto.sprite.y / this.cellSize);
  ...
  update(objeto) {
    if (!objeto || !objeto.sprite || isNaN(objeto.sprite.x)) return;
